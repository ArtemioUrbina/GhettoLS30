...
int read_LCD_buttons()
{
  ...
  return(parse_Key((analogRead(0) - adc_key_in)));
}  
...
uint32_t SelectHex(uint32_t startVal, uint32_t endVal, uint8_t charCount, uint8_t increment, int posy, const char *msg, uint32_t defaultVal = 0)
{
  char      buffer[24], chardefine[24];
  uint32_t  value = defaultVal, mask = 0, 
